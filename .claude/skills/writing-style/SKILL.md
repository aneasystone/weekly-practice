...
模式：`{具体痛点描述（带产品名/技术名）} → {自然引出项目} → {核心特性列表} → {本文目标（可选，禁止模板化，见下方红线）}`
...
模式：`{近期热点/社区现象} → {自然引出项目} → {核心特性列表} → {本文目标（可选，禁止模板化，见下方红线）}`
...
模式：`{时间节点 + 行业事件} → {唤起读者对相关技术的记忆} → {本文目标（可选，禁止模板化，见下方红线）}`

**开头结尾的"本文目标"句红线（最容易千篇一律的地方）**

上面三种模式末尾都有一个 `{本文目标}` 占位，但它是**可选项**，而且是生成文章里最容易写成套路的一处。作者真实的首篇（turbovec、openclaw、mcp 等）开头**几乎从不预告本文结构**——痛点/热点讲完、项目引出、特性列完，就直接进第一个 `## ` 正文章节了，不会专门写一句"本文先讲清楚 X 是什么、解决什么问题，然后动手跑起来"。

**禁止的套路句**（每次都长一个样，是 AI 腔的典型特征）：

- `今天是这个系列的第一篇，我们先把 X 是什么、解决什么问题讲清楚，然后动手把它跑起来`
- `本文将先介绍 X 的核心概念，再带大家一步步上手实践`
- `接下来，让我们先了解 X 的基本原理，然后通过一个示例来体验它的功能`

这些句子的共同毛病：把文章的章节目录用一句话复述了一遍，既没有新信息，也暴露了模板痕迹。读者不需要被预告"先讲是什么再讲怎么用"，往下看自然就知道了。

**正确做法（按优先级）**：

1. **直接进正文**（最常用，也最像作者）。特性列表/热点讲完，紧跟着就是 `## {第一个章节}`，不写任何过渡。参考 turbovec 入门篇：引出主角、列完特性，下一行就是 `## 与 FAISS 的对比定位`。
2. **用一句"具体的好奇钩子"收尾**，钩子要扣住这个项目独有的卖点，而不是复述结构。比如某项目主打"免费就能出片"，可以写 `免费就能做出真视频，这话光看 README 还不太敢信，得自己上手跑一遍才知道靠不靠谱`；主打"无需训练"，可以写 `不训练就能建好索引，听起来有点反直觉，它到底是怎么做到的`。钩子每篇都不一样，因为每个项目的卖点不一样。
3. 如果确实想点一句本文范围，**只点一个具体动作 + 具体产物**，不要罗列"是什么→怎么用→深入"这种全景。比如 `这篇我们就用零 key 把它跑起来，渲染出第一条视频`，到此为止，不要再加"然后理解它的原理"。

判定方法：写完开头最后一句，问自己——这句话换成另一个项目还成不成立？如果把项目名一换、这句话照样通顺（"先讲清楚 X 是什么再上手"对任何项目都成立），那就是套路句，删掉或换成扣住本项目卖点的钩子。
...
介绍篇开头提到项目时，要不要带一句"作者是某某"，取决于这个作者**是否真的知名**。判断标准（写之前花一分钟查 GitHub/主页）：

- **值得点名**：业界公认的知名开发者或团队、知名公司/实验室（如 Meta、微软研究院、某知名框架的原作者），或者作者本人就是读者熟悉的 IP。点名能给项目背书、增加可信度
- **不值得点名（默认情况）**：账号较新、followers 不多、除本项目外没有其他广为人知的作品的个人开发者。这种情况下硬写"作者是 xxx"既不能给项目加分，还占用了开头宝贵的注意力，读者根本不认识这个名字

判定方法：把作者名换成"某开发者"，开头这句话的信息量有没有损失？如果没有（读者本来也不认识），就直接删掉作者名，让项目的事实（star、特性、被报道）自己说话。拿不准就**不写**——介绍篇的主角是项目，不是作者。
...
8. **开头"本文目标"套路句排查**（系列首篇必查）：看开头最后一段有没有"先讲清楚 X 是什么、解决什么问题，然后动手跑起来""本文将先介绍……再带大家上手"这类预告章节结构的句子。判定方法：把项目名一换这句话是否照样成立，成立就是套路句，删掉或换成扣住本项目独有卖点的好奇钩子（详见 3.2 节"本文目标句红线"）
